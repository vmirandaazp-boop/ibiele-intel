...
// ✅ LLAVES INYECTADAS DIRECTAMENTE PARA EVITAR ERRORES DE CARGA
const supabaseUrl = "https://stenaxhdsfxrzhetetiz.supabase.co";
const supabaseAnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN0ZW5heGhkc2Z4cnpoZXRldGl6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAyMDc2NTgsImV4cCI6MjA4NTc4MzY1OH0.n57C-x2xVzACHisA5Jm4CWGJ0S9YDJQiEycaOrUI5wA";
      ...
      <header className="bg-[#003566] text-white p-4 shadow-2xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3">
          <div className="text-center md:text-left">
            ...
            <p className="text-[8px] md:text-[10px] font-bold opacity-70 uppercase mt-1">{time.toLocaleTimeString()} • HQ COMMAND</p>
          </div>
        ...
        {activeTab === "dashboard" && (
          <div className="animate-in fade-in duration-500 space-y-6">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="bg-slate-500 p-6 rounded-xl md:rounded-[40px] text-white border-b-8 border-slate-700 shadow-xl opacity-90">
                  ...
                  <div className="flex justify-between items-end">
                    <div><p className="text-4xl font-black italic">$1,104.32</p><p className="text-[8px] font-bold uppercase">Cierre Enero</p></div>
                    <div className="text-[10px] font-black bg-white/10 px-3 py-1 rounded-full">ARCHIVO</div>
                  </div>
               </div>
               <div className="bg-[#003566] p-6 rounded-xl md:rounded-[40px] text-white border-b-8 border-blue-900 shadow-xl">
                  ...
                  <div className="flex justify-between items-end">
                    <div><p className="text-4xl font-black italic text-blue-400">${totalRevenue.toFixed(2)}</p><p className="text-[8px] font-bold uppercase">Target: ${dailyTarget.toFixed(2)}</p></div>
                    <div className="text-[10px] font-black bg-blue-500 px-3 py-1 rounded-full animate-pulse">VIVO</div>
                  </div>
               </div>
            </section>
            ...
            <section className={`p-4 md:p-8 rounded-xl md:rounded-[40px] shadow-xl border-4 ${editingId ? 'bg-blue-50 border-blue-500' : 'bg-white border-slate-300'}`}>
              <h3 className="text-xs font-black text-[#003566] uppercase mb-4 border-b-2 pb-2 italic">{editingId ? "⚡ EDITANDO REGISTRO" : "📝 NUEVO REGISTRO"}</h3>
            ...
            {/* BITÁCORA */}
            <section className="bg-white rounded-xl md:rounded-[40px] p-4 md:p-8 border-2 border-slate-300 shadow-xl overflow-x-auto">
              ...
              <div className="min-w-[500px] space-y-2">
                {febData.map(item => (
                  <div key={item.id} onClick={() => handleEdit(item)} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-200 hover:bg-blue-50 cursor-pointer transition-all">
                    <div className="flex-1"><p className="font-black text-[#003566] text-sm md:text-base uppercase truncate pr-4">{item.topic || "SIN TÍTULO"}</p><p className="text-[8px] font-bold text-slate-400">{(item.date || "").split("-").reverse().slice(0,2).join("/")} • {((item.views || 0)/1000000).toFixed(2)}M VISTAS</p></div>
                    <div className="flex items-center gap-4"><p className="font-black text-green-600 text-xl md:text-3xl">${Number(item.revenue || 0).toFixed(2)}</p><button onClick={(e)=>{e.stopPropagation(); deleteRow(item.id);}} className="text-red-400 text-xl">🗑️</button></div>
                  </div>
                ))}
              </div>
            </section>
